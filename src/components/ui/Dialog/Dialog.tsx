...
const dialogVariants = cva(
  'relative transform rounded-lg bg-white text-left shadow-xl transition-all dark:bg-gray-800 w-full',
  {
    variants: {
      size: {
        sm: 'max-w-sm',
...
export const Dialog = ({ open, onClose, children, size, className }: DialogProps) => (
      ...
      <HeadlessDialog as="div" className="relative z-50" onClose={onClose}>
        ...
        <div className="fixed inset-0 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            ...
            >
              <HeadlessDialog.Panel className={cn(dialogVariants({ size }), className)}>
                {children}
...
export const DialogContent = ({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('p-4 overflow-y-auto max-h-[calc(100vh-16rem)]', className)} {...props}>
    {children}
  </div>
);

export const DialogFooter = ({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('flex items-center justify-end space-x-2 p-4 border-t', className)} {...props}>
    {children}
  </div>
);
